...
import { Form, FormGroup, FormControl,
  Grid, Row, Col, ControlLabel, Panel,
...
const Settings = React.createClass({
  ...
  handleSubmit(e) {
    ...
    if (user.perm === 0 && this.state
      .newPass1 === this.state.newPass2
    ) {
  ...
  changeOn(e) {
    if (e.target.name ===
      "currentPass") this.props.resetPasswordMessage();
    if (e.target.name ===
      "newPass2") {
      if (e.target.value === this.state
        ...
        this.setState({ validate: "error",
          validateMessage: "Error! Passwords must match" });
      }
    }
    this.setState({
  ...
  render() {
    const { orgName, user } = this.props
      .organization;
    const permName = { 0: "an Admin",
      1: "a Producer", 2: "a User" };
    return (
